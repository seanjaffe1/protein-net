...
parser.add_argument("--data_dir", help="Directory to place new data files")
...
parser.add_argument("--out_file", default='scores.csv', help="File to write db to")
...
score_types = [fa_atr,
         ...
         yhh_planarity,
         ref, rama_prepro]
column_names=['pdb_id', 'total','len','seq'] + [str(s)[10:] for s in score_types]
...
for pdbid in ids_to_query:
    ...
    for j in range(1, pose.total_residue() + 1):
        ...
        try:
            ...
            psis[j-1] = np.nan
    np.savetxt(data_dir + '/' + str(pdbid) + '.angles.txt', [phis, psis],delimiter=',')
    length = pose.total_residue()
    sub_scores = [pose.energies().total_energies()[sub_score] for sub_score in score_types]
    temp_db = pd.DataFrame([[pdbid, total, int(length), pose.sequence()] + sub_scores], columns=column_names, index=[len(curr_db)])
    curr_db = pd.concat([curr_db, temp_db], axis=0)
    
# Write db to out file
curr_db.to_csv(data_dir + '/' + out_file)
