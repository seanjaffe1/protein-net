...
class CNNModel1(object):
    ...
    def __init__(self, dropout_prob=0.0, batch_norm=False, is_trianing=True):
        ...
        self.W_conv3 = weight_variable([5, 1, 20, 24])
        self.b_conv3 = bias_variable([24])
        self.h_conv3 = tf.nn.relu(conv2d(self.h_conv2, self.W_conv3,3) + self.b_conv3)
                
        #TODO why 1840?
        self.W_fc1 = weight_variable([316800, 1164]) # in, out
        self.b_fc1 = bias_variable([1164])
        
        self.h_conv3_flat = tf.reshape(self.h_conv3, [-1,316800]) 
