...
class CNNModel2(object):
    ...
    def __init__(self, dropout_prob=0.0, batch_norm=False, is_trianing=True):
        ...
        self.W_conv4 = weight_variable([5, 1, 30, 30])
        self.b_conv4 = bias_variable([30])
        self.h_conv4 = tf.nn.relu(conv2d(self.h_conv3, self.W_conv4,3) + self.b_conv4)

        self.W_conv5 = weight_variable([5, 1, 30, 35])
        self.b_conv5 = bias_variable([35])
        self.h_conv5 = tf.nn.relu(conv2d(self.h_conv4, self.W_conv5,3) + self.b_conv5)
        
        #TODO why 1840?
        self.W_fc1 = weight_variable([45500, 1164]) # in, out
        self.b_fc1 = bias_variable([1164])
        
        self.h_conv5_flat = tf.reshape(self.h_conv5, [-1,45500]) 
        ...
        y = tf.add(tf.matmul(self.h_fc4_drop, self.W_fc5), self.b_fc5, name='y') 
